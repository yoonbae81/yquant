...
본 문서는 **yq-gateway**, **yq-blue**, **yq-green** 3개의 Worker를 활용한 인프라 구성 및 배포 운영 전략을 기술합니다.

## 1. 아키텍처 개요 (Blue/Green Architecture)

시스템은 트래픽을 관장하는 게이트웨이 Worker(`gateway`)와 실제 애플리케이션이 구동되는 쌍둥이 Worker 노드(`blue`/`green`)로 구성됩니다.
...
### 2.1. `yq-gateway` (Gateway 노드)
*   **역할**: 공용 인터넷과 내부망 사이의 문지기 (Ingress Gateway)
*   **주요 컴포넌트**:
    *   **HAProxy**: 외부 트래픽(TradingView Webhook)을 받아 Active 상태인 Worker(`blue` 또는 `green`)로 라우팅합니다.
    *   **Let's Encrypt (Certbot)**: SSL/TLS 인증서를 발급 및 갱신하여 TLS Termination을 수행합니다.
    *   **Firebird DB**: `gateway` Worker에서 구동되며, 공통 마스터 데이터(종목 카탈로그), 예약 주문, 증권사 토큰 및 매매 이력을 저장하는 공유 데이터베이스입니다.
*   **특징**:
...
*   **Valkey 구성 (Independent Pub/Sub)**:
    ...
    *   **Shared DB 연동**: 모든 Worker는 `gateway` Worker의 **Firebird DB**에 접속하여 영구 데이터를 공유합니다. 종목 카탈로그는 구동 시 DB에서 로드하여 로컬 메모리로 캐싱합니다. (이후 `blue/green`은 최신 정보를 DB와 동기화하며 동작)

## 3. 네트워크 및 접근 제어

### 3.1. 외부 트래픽 (TradingView Webhook)
*   **경로**: `Internet` → `yq-gateway` (443) → `Active Worker` (Internal IP:WebPort)
*   **보안**:
    *   `yq-gateway`에서 SSL 복호화(Termination) 수행.
    *   `gateway`과 `compute` Worker 간은 내부 사설망 통신.
...
3.  **전환 (Switch)**
    *   검증이 완료되면 `yq-gateway`의 HAProxy 설정을 수정하여 트래픽 백엔드를 `blue`에서 `green`으로 변경합니다.
...
| :--- | :--- | :--- | :--- | :--- |
| **`yq-gateway`** | **80, 443** | **외부 서비스** (HAProxy) | Internet → HAProxy | **VCN + firewalld Open (Any)** |
| `yq-gateway` | 3050 | Firebird DB | Blue/Green → Dock | VCN subnet (10.0.0.0/24) |
| `yq-gateway` | 41641 | Tailscale | P2P VPN | UDP Open (Any) |
...
*   **`switch-active.sh`**: `yq-gateway` 서버에서 HAProxy 설정을 변경하여 Active Worker를 전환합니다.
...
2.  **Valkey 장애**: 
    ...
    *   Storage Valkey (`gateway` Worker) 장애 시에는 신규 진입이나 카탈로그 동기화가 제한될 수 있으나, 이미 로드된 메모리 데이터로 트레이딩은 지속 가능합니다.

3.  **Gateway 노드 장애**: 
    *   DNS를 `blue` 또는 `green`의 공인 IP로 임시 변경하고, 해당 Worker의 방화벽을 개방하여 비상 운영합니다.

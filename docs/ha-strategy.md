...
| :--- | :--- | :--- | :--- |
| **`yq-port`** | E2.Micro (1 OCPU, 1GB) | 시스템 관문 및 데이터 저장소 | HAProxy, Valkey (Storage), Sentinel |
...
*   **Valkey 이원화**:
    ...
    *   **Storage Valkey**: `port`에 위치하며 증권사 인증 토큰 및 공통 데이터 보관. 엔진 서버가 교체되어도 상태 유지.
...
3.  **Smoke Test**: `yq-port`에서 제공하는 스테이징 전용 경로(예: `8080` 포트)를 통해 `green` 노드의 대시보드와 로그를 확인합니다.
...
*   **`switch-active.sh`**: `yq-port` 서버에서 Active 노드를 전환할 때 사용합니다.
...
3.  **Port 장애**: 인프라 수준의 복구가 필요하며, 복구 전까지 `blue`/`green` 중 마스터를 수동으로 지정하여 운영할 수 있습니다.

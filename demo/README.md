...
- **Docker & Docker Compose**: 데모 인프라(Valkey, KIS-Mock)를 실행하는 데 필요합니다.
...
데모 환경은 운영 데이터와의 간섭을 방지하기 위해 전용 Valkey 인스턴스와 KIS Mock 서버가 필요합니다.
...
2. 인프라를 시작합니다:
   ...
   - **Valkey (Message)**: 6380 포트 (이벤트 및 상태 관리용)
   - **Valkey (Token)**: 6381 포트 (금융사 인증 토큰 저장용)
...
1. **국내/해외 종목 카탈로그 동기화**:
   데모는 전용 Valkey를 사용하므로, 먼저 종목 메타데이터를 채워야 합니다.
   ```bash
   # 루트 디렉토리에서 실행
...
3. **대시보드 접속**:
   브라우저에서 `http://localhost:2000` (또는 설정된 포트)에 접속합니다.
